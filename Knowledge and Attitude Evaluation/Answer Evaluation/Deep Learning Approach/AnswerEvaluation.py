...
class AnswerEvaluation:
    ...
    def calCosineSimilarity(self, model_ans, ans):
        model_ans_embed = self.bert(self.preprocess([model_ans]))["pooled_output"]
    ...
    def predict(self, sample_answers, candidate_answer):
        ...
        for sample_answer in sample_answers:
            print("\n------------------------------------------------------------------------")
            length_ratio = self.get_length_ratio(sample_answer, candidate_answer)
            cosine_similarity = self.calCosineSimilarity(sample_answer, candidate_answer)

            testdf = pd.DataFrame(columns=['Model_Answer', 'Answer', 'LengthRatio', 'Cosine_Similarity'])
            testdf.loc[0] = [sample_answer, candidate_answer, length_ratio, cosine_similarity]

            prediction = self.model.predict([testdf["Model_Answer"].iloc[0:1], testdf["Answer"].iloc[0:1], np.asarray(testdf[['LengthRatio', 'Cosine_Similarity']].iloc[0:1]).astype(np.float32)])
